...
void on_read(uv_stream_t *stream, ssize_t nread, const uv_buf_t *buf) {
  ...
  std::size_t length = 0;
  if (message.consume_front("Content-Length: ") &&
      !message.consumeInteger(10, length) &&
...
void listen(Callback callback) {
  ...
  UV_CHECK_RESUlT(
      uv_read_start(uv_cast<uv_stream_t>(in), net::on_alloc, net::on_read));
}
...
void listen(const char *ip, unsigned int port, Callback callback) {
  ...
  auto on_connection = [](uv_stream_t *server, int status) {
    ...
    UV_CHECK_RESUlT(uv_read_start(uv_cast<uv_stream_t>(client), net::on_alloc,
                                  net::on_read));
  };
...
void spawn(llvm::StringRef path, llvm::ArrayRef<std::string> args,
           Callback callback) {
  static uv_pipe_t in;
  static uv_pipe_t out;
  ...
  UV_CHECK_RESUlT(
      uv_read_start((uv_stream_t *)&out, net::on_alloc, net::on_read));

  /// FIXME: This implementation is not correct.
  auto on_read = [](uv_stream_t *stream, ssize_t nread, const uv_buf_t *buf) {
    if (nread > 0) {
      log::warn("{0}",
                llvm::StringRef{buf->base, static_cast<std::size_t>(nread)});
    } else if (nread < 0) {
      if (nread != UV_EOF) {
        log::fatal("An error occurred while reading: {0}", uv_strerror(nread));
      }
