...
cl::opt<std::string> mode("mode", cl::cat(category),
                          cl::value_desc("pipe|socket|indexer"),
                          cl::desc("The mode of clice, default is pipe, socket "
                                   "is usually used for debugging"));

cl::opt<std::string>
    config_path("config", cl::cat(category), cl::value_desc("path"),
                cl::desc("The path of the clice config file, if not specified, "
                         "the default config will be used"));

cl::opt<std::string> resource_dir(
    "resource-dir", cl::cat(category), cl::value_desc("path"),
    cl::desc(
        R"(The path of the clang resource directory, default is "../../lib/clang/version")"));
...
bool checkArguments(int argc, const char **argv) {
  ...
  cl::ParseCommandLineOptions(
      argc, argv, "clice is a new generation of language server for C/C++");
  ...
  if (config_path.empty()) {
    ...
    if (auto result = config::load(argv[0], path); result) {
      ...
      log::warn("Failed to load configuration file from: {} because {}", path,
                result.error());
      return false;
    }
  }
  ...
  if (resource_dir.empty()) {
    log::info(
        "No resource directory specified, using default resource directory");
    // Try to initialize default resource directory
    if (auto result = fs::init_resource_dir(argv[0]); !result) {
      log::warn("Cannot find default resource directory, because {}",
                result.error());
      return false;
    }
  } else {
...
int main(int argc, const char **argv) {
  ...
  llvm::setBugReportMsg(
      "Please report bugs to https://github.com/clice-project/clice/issues and "
  ...
  const auto loop = [&](const json::Value value) -> async::Task<> {
