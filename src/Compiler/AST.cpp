...
ASTInfo::toLocalRange(clang::SourceRange range) {
  auto [begin, end] = range;
  assert(begin.isValid() && end.isValid() && "Invalid source range");
  assert(begin.isFileID() && end.isValid() &&
         "Input source range should be a file range");

  if (begin == end) {
    auto [fid, offset] = getDecomposedLoc(begin);
    return {fid, {offset, offset + getTokenLength(SM, end)}};
  } else {
    auto [beginFID, beginOffset] = getDecomposedLoc(begin);
    auto [endFID, endOffset] = getDecomposedLoc(end);
    if (beginFID == endFID) {
      return {beginFID, {beginOffset, endOffset + getTokenLength(SM, end)}};
    } else {
      auto content = getFileContent(beginFID);
      return {beginFID, {beginOffset, static_cast<uint32_t>(content.size())}};
    }
  }
}
...
ASTInfo::toLocalExpansionRange(clang::SourceRange range) {
  ...
  if (begin == end) {
    ...
    return toLocalRange(
        clang::SourceRange(getExpansionLoc(begin), getExpansionLoc(end)));
  }
}
