...
std::uint32_t computeBounds(llvm::StringRef content) {
    ...
    while(!lexer.LexFromRawLexer(token)) {
        ...
        if(token.isAtStartOfLine() && token.getKind() == clang::tok::hash) {
            /// If we encounter a `#` at the start of a line, it must be a directive.
            isInDirective = true;
            continue;
        } else if(token.isAtStartOfLine() && token.getKind() == clang::tok::identifier &&
                  token.getRawIdentifier() == "module") {
            if(!lexer.LexFromRawLexer(token) && token.getKind() == clang::tok::semi) {
    ...
    return end.getLocation().getRawEncoding() + end.getLength();
}
