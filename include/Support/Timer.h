...
template <typename T> class TimerRegion {
public:
  explicit TimerRegion(Timer<T> &timer) : timer(timer) { timer.start(); }

  TimerRegion(const TimerRegion &) = delete;
  TimerRegion &operator=(const TimerRegion &) = delete;
  TimerRegion(TimerRegion &&) = delete;

  ~TimerRegion() { timer.get().stop(); }

private:
  std::reference_wrapper<Timer<T>> timer;
};

template <typename T> TimerRegion(Timer<T>) -> TimerRegion<T>;
}; // namespace clice
