...
constexpr inline bool is_directly_binarizable_v = [] {
  if constexpr (std::is_integral_v<T> || refl::reflectable_enum<T>) {
    ...
    return refl::member_types<T>::apply([]<typename... Ts>() {
      return (is_directly_binarizable_v<Ts> && ...);
    });
  } else {
    return false;
  }
...
template <typename T> consteval auto binarify();

template <typename T> using binarify_t = typename decltype(binarify<T>())::type;
...
template <typename T> consteval auto binarify() {
  if constexpr (is_directly_binarizable_v<T>) {
    ...
    return tuple_to_list_t<T>::apply([]<typename... Ts> {
...
template <typename T> struct Section {
...
template <typename T, typename Primary = void> consteval auto layout() {
  if constexpr (is_directly_binarizable_v<T>) {
    return std::tuple<>();
  } else if constexpr (std::is_same_v<T, std::string>) {
...
/// layout is unique.
template <typename T> using layout_t = tuple_uniuqe_t<decltype(layout<T>())>;

template <typename T> struct Packer {
  ...
  template <typename Object> void init(const Object &object) {
    if constexpr (std::same_as<Object, std::string>) {
      ...
      std::get<Section<typename Object::value_type>>(layout).total +=
          object.size();
  ...
  template <typename Object>
    requires(is_directly_binarizable_v<Object> &&
             !refl::reflectable_struct<Object>)
  ...
  array<V> write(const Object &object) {
    ...
    for (std::size_t i = 0; i < size; ++i) {
      ::new (buffer.data() + offset +
             i * sizeof(binarify_t<V>)) auto{write(object[i])};
    }
  ...
  std::array<char, sizeof(binarify_t<Object>)> write(const Object &object) {
    ...
    refl::foreach (result, object, [&](auto &lhs, auto &rhs) {
      auto offset =
          reinterpret_cast<char *>(&lhs) - reinterpret_cast<char *>(&result);
...
template <std::size_t N> struct fixed_string : std::array<char, N + 1> {
  template <std::size_t M> constexpr fixed_string(const char (&str)[M]) {
  ...
  constexpr operator std::string_view() const { return {this->data(), N}; }
};
...
template <typename T> struct Proxy {
  ...
  template <fixed_string name> auto get() const {
...
/// Binirize an object.
template <typename Object> auto serialize(const Object &object) {
...
template <typename Object> Object deserialize(Proxy<Object> proxy) {
